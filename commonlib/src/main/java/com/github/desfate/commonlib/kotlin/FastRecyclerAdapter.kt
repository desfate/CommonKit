...
class FastRecyclerAdapter <T>(var items: List<T>,
                              layoutCallBack: LayoutCallBack ?= null,
                              convertCallBack: ConvertCallBack<T> ?= null) : RecyclerView.Adapter<FastViewHolder> (){
    ...
    override fun onBindViewHolder(p0: FastViewHolder, p1: Int) {
        convertCallBacks.invoke(p0, items[p1], p1)
    }

    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): FastViewHolder {
        return FastViewHolder.getViewHolder(p0, layoutCallBacks.invoke(p1))
    }
